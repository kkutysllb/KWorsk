...
show_help() {
    ...
    echo "  all         深度清理（含node_modules/.cache、.oclaw运行数据）+ build"
    echo "  (默认)      等同于 cache"
    echo ""
    echo "清理示例:"
...
start_gateway() {
    ...
    if [ "$mode" != "prod" ]; then
        gateway_flags="--reload --reload-include='*.yaml' --reload-include='.env' --reload-exclude='*.pyc' --reload-exclude='__pycache__' --reload-exclude='sandbox/' --reload-exclude='.oclaw/'"
    else
...
_export_compose_env() {
    export KKOCLAW_HOME="${KKOCLAW_HOME:-$REPO_ROOT/backend/.oclaw}"
...
clean_resources() {
    ...
    if [ "$level" = "all" ]; then
        ...
        # 运行时用户数据（线程、内存等）
        _rm_report ".oclaw" "KKOCLAW 运行时数据"
    fi
