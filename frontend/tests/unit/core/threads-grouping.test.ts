...
describe("resolveThreadWorkModeId", () => {
  ...
  test("prefers explicit work_mode_id when present", () => {
    ...
    seedLocalStorage({
      [`${THREAD_WORK_MODE_KEY_PREFIX}t-1`]: "custom-research",
      [`${THREAD_AGENT_KEY_PREFIX}t-1`]: "lead_agent",
    });

    expect(resolveThreadWorkModeId(thread)).toBe("custom-research");
  });

  test("falls back to agent_name reverse-resolve when work_mode_id absent", () => {
    // An agent_name matching no builtin mode resolves to a transient mode
    // whose id is the agent_name itself (no coding_agent mapping exists now).
    const thread = makeThread("t-2");
    seedLocalStorage({
      [`${THREAD_AGENT_KEY_PREFIX}t-2`]: "research-agent",
    });

    expect(resolveThreadWorkModeId(thread)).toBe("research-agent");
  });
...
describe("groupThreadsByWorkMode", () => {
  ...
  test("groups threads by resolved mode id", () => {
    seedLocalStorage({
      ...
      [`${THREAD_WORK_MODE_KEY_PREFIX}b`]: "custom-research",
    ...
    // task (builtin, order 0) before the custom mode (transient, order 999)
    expect(groups[0]!.workModeId).toBe("task");
    expect(groups[0]!.threads.map((t) => t.thread_id)).toEqual(["a", "c"]);
    expect(groups[1]!.workModeId).toBe("custom-research");
  ...
  test("respects WorkMode.order for sort stability", () => {
    ...
    seedLocalStorage({
      [`${THREAD_WORK_MODE_KEY_PREFIX}x`]: "custom-research",
