...
const GUIDE_CONTENT: Record<string, {
  ...
  dingtalk: {
    intro: "钉钉（DingTalk）是阿里巴巴集团打造的企业级智能移动办公平台。配置后用户可通过钉钉机器人或群聊与 KWorks 交互。",
    steps: [
  ...
  discord: {
    intro: "Discord 是全球广受欢迎的社区通讯平台。配置后可通过 Discord Bot 与 KWorks 在频道或私信中对话。",
    steps: [
  ...
  feishu: {
    intro: "飞书（Feishu/Lark）是字节跳动旗下企业协作平台，支持流式响应。配置后用户可在飞书群聊或机器人中与 KWorks 对话。",
    steps: [
  ...
  slack: {
    intro: "Slack 是全球流行的团队沟通工具。配置后可通过 Slack Bot 与 KWorks 在频道或私信中对话。",
    steps: [
  ...
  telegram: {
    intro: "Telegram 是注重隐私与速度的即时通讯平台。配置后用户可通过 Telegram Bot 与 KWorks 对话。",
    steps: [
  ...
  wechat: {
    intro: "微信（WeChat）是中国最广泛使用的即时通讯平台。配置后用户可通过微信公众号或企业微信与 KWorks 交互。",
    steps: [
  ...
  wecom: {
    intro: "企业微信（WeCom）是腾讯推出的专业企业通讯与办公平台。配置后用户可通过企业微信机器人或应用与 KWorks 交互，支持流式响应。",
    steps: [
