 ...
 * that ResizeObserver emits on the very first frame, and the matching empty
 * render that happens during hydration or when the data flips from empty to
 * populated.
 */
function ResponsiveChart({ children }: { children: React.ReactElement }) {
  const [ready, setReady] = useState(false);
  useEffect(() => {
    // Defer one frame so the parent has finished laying out before the chart
    // attaches its ResizeObserver. Without this, Recharts v3 logs the
    // `width(-1)/height(-1)` warning on the very first measurement.
