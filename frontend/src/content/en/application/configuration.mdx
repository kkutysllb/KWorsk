---
title: Configuration
description: KKOCLAW App is configured through two files and a set of environment variables. This page covers the application-level configuration that most operators need to set up before deploying.
---
...
KKOCLAW App is configured through two files and a set of environment variables. This page covers the application-level configuration that most operators need to set up before deploying.
...
Install Ollama provider: `cd backend && uv add 'kkoclaw-harness[ollama]'`
...
Install: `cd backend && uv add 'kkoclaw-harness[aio-sandbox]'`
...
tools:
  ...
  - use: kkoclaw.community.ddg_search.tools:web_search_tool # default, no key required
  ...
  # File operations
  - use: kkoclaw.sandbox.tools:ls_tool
  - use: kkoclaw.sandbox.tools:read_file_tool
  - use: kkoclaw.sandbox.tools:glob_tool
  - use: kkoclaw.sandbox.tools:grep_tool
  - use: kkoclaw.sandbox.tools:write_file_tool
  - use: kkoclaw.sandbox.tools:str_replace_tool
  - use: kkoclaw.sandbox.tools:bash_tool
```

### Thread state persistence (checkpointer)

By default, KKOCLAW uses an SQLite checkpointer for thread state persistence:

```yaml
checkpointer:
  type: sqlite
  connection_string: checkpoints.db # stored in backend/.oclaw/
```
...
checkpointer:
  type: postgres
  connection_string: postgresql://user:password@localhost:5432/kkoclaw
```
