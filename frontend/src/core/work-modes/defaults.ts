 ...
 *
 * These mirror the backend's shipped ``work_modes.modes`` config (task).
 * The frontend keeps its own copy so the UI can render instantly before
 * the API call resolves, and so it has a fallback when the backend is
 * unreachable (see ``FALLBACK_WORK_MODES`` in ``./api``).
 *
