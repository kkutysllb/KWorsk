...
commit_and_tag() {
  ...
  if git diff --cached --quiet --; then
    # Nothing to stage: the release content (version bump + lockfiles +
    # changelog) is already present in HEAD — either from a manual commit
    # or a prior run that didn't finish tagging. Since update_versions is
    # idempotent and just confirmed the working tree is at $VERSION, HEAD
    # is a valid commit to tag. Fall through to tagging instead of aborting.
    warn "No new release changes to commit; content for $VERSION is already in HEAD."
    warn "Will tag current HEAD ($(git rev-parse --short HEAD)) as $TAG."
  else
